...
- Ejecutado (UTC): 2026-01-04T04:49:07.933818+00:00
...
- Conteos: clases=47, objProps=43, dataProps=8, instancias/otros=0

## Hallazgos críticos (Gist)
- Importación no versionada de Gist: cambiar a `gistCore14.0.0` para estabilidad.
- Reuso explícito de propiedades Gist: incorporar `rdfs:subPropertyOf`/`owl:inverseOf` para gobernanza (`gist:isGovernedBy`), geo-contención (`gist:isGeoContainedIn`), parte‑todo (`gist:isPartOf`) y participación (`gist:hasParticipant/hasParty`).
- Magnitudes: valores numéricos sin unidad deberían migrar a `gist:Magnitude` cuando se requiera comparación/cálculo o multiunidad.
- Temporalidad: estados “actuales” deberían considerar `gist:TemporalRelation` si habrá historial/vigencia.
...
- owl:versionIRI: `<https://gorenuble.gob.cl/ontology/goreNubleOntology1.0.0>`
- Recomendaciones:
  - Importar `https://w3id.org/semanticarts/ontology/gistCore14.0.0` (versionado) en vez de `.../gistCore`.
...
- Recomendaciones:
  - Tensión Gist: podrías tipar organizaciones con categorías (instancias de `gnub:OrganizationalUnitType`) en vez de subclases de `gist:Organization`, salvo que necesites inferencias fuertes por tipo.
...
- Alineamiento Gist: **REVIEW**
- Recomendaciones:
  - Misma tensión que `gnub:Division`.
...
- Alineamiento Gist: **REVIEW**
- Recomendaciones:
  - La etiqueta suena a entidad, pero es una taxonomía (‘tipo de entidad’). Considerar renombrar a `gnub:ApplicantInstitutionType` para alinearse con el paradigma Category.
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Reusar `gist:isGeoContainedIn` (la relación canónica en Gist) o declarar `gnub:containsGeoRegion owl:inverseOf gist:isGeoContainedIn` y, si quieres inferencia explícita, `a owl:TransitiveProperty`.
  - Ajustar dominio/rango a `gist:GeoLocation` si la intención es contención geográfica general (en Gist la propiedad aplica a `GeoLocation`).
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Si significa propiedad/posesión: mapear a `gist:owns` (`gnub:isOwnedBy owl:inverseOf gist:owns`). Si significa ‘responsable institucional’, renombrar (p.ej. `gnub:isStewardedBy`) o mapear a `gist:isGovernedBy`.
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Cae en gobernanza. `rdfs:subPropertyOf gnub:governs`.
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Cae en gobernanza. Recomiendo introducir `gnub:governs owl:inverseOf gist:isGovernedBy` y hacer `gnub:regulates rdfs:subPropertyOf gnub:governs`.
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Si requieres montos/fechas: modelar como `gist:Transaction` (subclase de `gist:Event`) + `gist:hasGiver/hasRecipient` + `gist:hasMagnitude` monetaria. Si no, al menos `rdfs:subPropertyOf gnub:governs` (gobernanza financiera).
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Similar a `gnub:funds`: si hay trazabilidad presupuestaria, reificar como `gist:Transaction`/evento; si no, colgar de `gnub:governs`.
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Si se modela como relación estable de autoridad/fiscalización, también puede colgar de gobernanza (`gnub:governs`). Si necesitas auditorías con fecha/resultado, modelar eventos (`gist:Event`) y participantes.
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Alineación de participación: `gnub:hasApplicant rdfs:subPropertyOf gist:hasParty` (si restringes a Org/Person) o `gist:hasParticipant`.
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Si esta propiedad sólo clasifica, considerar `gist:isCategorizedBy` (manteniendo la propiedad específica sólo si aporta claridad/compatibilidad).
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - `xsd:decimal` sin unidad es frágil (UTM vs CLP). Preferir patrón `gist:Magnitude` monetaria con unidad (`gist:UnitOfMeasure`).
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Porcentaje suele requerir aclarar unidad (0–1 vs 0–100). Si se comparará/calculará, modelar como `gist:Magnitude` (aspecto ‘ratio/percentage’) + unidad definida en reference data.
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Si la unidad importa, preferir duración como `gist:Magnitude` (`_Aspect_duration`) + unidad (mes).
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Alineación de participación: `gnub:isEvaluatedBy rdfs:subPropertyOf gist:hasParticipant` (y si sólo Org/Person, `gist:hasParty`).
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Declarar `gnub:hasLegalMandate rdfs:subPropertyOf gist:isGovernedBy` (un mandato gobierna/dirige conducta).
  - Si necesitas vigencia/historial, reificar aplicación del mandato con `gist:TemporalRelation`.
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Declarar `gnub:belongsToLegalDocument rdfs:subPropertyOf gist:isPartOf` para alinearse con parte‑todo Gist.
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Opcional: declarar `gnub:binds owl:inverseOf gnub:hasLegalMandate` (si interpretas binds como ‘es gobernado por’ en dirección inversa).
...
- Alineamiento Gist: **IMPROVE**
- Recomendaciones:
  - Declarar `gnub:establishes owl:inverseOf gnub:derivesFromArticle` (coherencia navegable).
...
### Instancias / reference data
## Roadmap de mejora (priorizado)
### 1) Alineamiento declarativo (sin tocar datos)
- Versionar import de Gist.
- Declarar mapeos:
  - `gnub:containsGeoRegion owl:inverseOf gist:isGeoContainedIn` (y/o usar directo `gist:isGeoContainedIn`).
  - `gnub:belongsToLegalDocument rdfs:subPropertyOf gist:isPartOf`.
  - `gnub:hasLegalMandate rdfs:subPropertyOf gist:isGovernedBy`.
  - `gnub:hasApplicant rdfs:subPropertyOf gist:hasParty` y `gnub:isEvaluatedBy rdfs:subPropertyOf gist:hasParticipant`.

### 2) Modelado rico
- Reificar financiamiento/asignación con `gist:Transaction` si necesitas montos/fechas y actores.
- Migrar umbrales/plazos/% a `gist:Magnitude` con unidad explícita.
- Considerar `gist:TemporalRelation` para estados que cambian.
